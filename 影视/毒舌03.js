...
function extractPlaySource(html) {
  const text = String(html || "");
  const match = text.match(/(?:const|var|let|window\.)?\s*playSource\s*=\s*\{([\s\S]*?)\};/i);
  if (!match) return { url: "", type: "" };
  const body = decodeUnicodeEscapes(match[1]);
  // 兼容 src/url 键名既可带引号也可不带引号两种写法（实测本站为  src: "..." 不带引号）
  const urlMatch = body.match(/\b(?:src|url)\b\s*:\s*['"]([^'"]+)['"]/i);
  const typeMatch = body.match(/\btype\b\s*:\s*['"]([^'"]+)['"]/i);
  return {
